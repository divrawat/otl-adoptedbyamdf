
export const DOMAIN = 'http://localhost:3000';

export const IMAGES_DOMAIN = 'https://www.operationtruelove.in.net';
// export const DOMAIN = 'https://operationtruelove.adoptedbyamurderousdukefamily.in.net';


export const MANGA_DESCRIPTION = `<a href="${DOMAIN}"><u>Operation True Love</u></a> is a webtoon that revolves around a high school student named Seojin who devises a plan to win the heart of her crush, Sunjin. With the help of her friends, Seojin navigates through various comedic and heartfelt situations as she tries to execute her romantic strategy. The story explores themes of friendship, love, and self-discovery amidst the challenges of adolescence.`;

export const MANGA_NAME = 'Operation True Love';
export const MANGA_AUTHOR = 'kkokkalee';
export const MANGA_RELEASE = '2019';
export const MANGA_STATUS = 'Ongoing';
export const MANGA_ARTIST = 'kkokkalee';
export const MANGA_STUDIO = 'None';
export const MANGA_GENRE = 'Drama, Romance, Comedy';

export const CHAPTER_PREFIX = 'operation-true-love-chapter';
export const IMAGE_PREFIX = 'images/operation-true-love';
export const NEXT_PREVIOUS_PREFIX = 'manga/operation-true-love-chapter';
export const URL_PREFIX = 'manga/operation-true-love-chapter';

export const AUTHOR_PAGE = `${DOMAIN}/operation-true-love-team`;
export const LOGO_URL = `${DOMAIN}/logo.webp`;
export const COVER_IMG = `${DOMAIN}/cover.webp`;
export const BEHIND_COVER_IMG = "https://static1.srcdn.com/wordpress/wp-content/uploads/2024/01/operation-true-love-s-three-main-cahracters-on-a-bench.jpg";


// Navbar
export const logo = <img src={`${LOGO_URL}`} alt="Logo" className="h-[85px] w-[145px] mr-5 md:ml-0 ml-4 md:pb-2 md:my-0 my-1" />

export const APP_NAME = 'Operation True Love';
export const NavbarName = "Operation True Love";

export const APP_DESCRIPTION = `Read ${MANGA_NAME} manhwa or manga all chapters online. Seojin who devises a plan to win the heart of her crush, Sunjin. With the help of her friends, Seojin navigates through various comedic and heartfelt situations as she tries to execute her romantic strategy. The story explores themes of friendship, love, and self-discovery amidst the challenges of adolescence.`;

export const DISQUS_SHORTNAME = "my-cms-7";


export const MANGA_SUMMARY = [
    {
        id: 1,
        content: "Operation True Love is a charming webtoon that delves into the life of Seojin, a typical high school student who harbors a deep crush on Sunjin, a popular boy at school. Determined to win his heart, Seojin concocts a meticulous plan titled Operation True Love with the enthusiastic support of her close-knit group of friends."
    },
    {
        id: 2,
        content: "The webtoon unfolds through a series of comedic and heartwarming episodes, each showcasing Seojin's earnest attempts to execute her romantic strategy. From awkward encounters to unexpected obstacles, Seojin navigates the complexities of teenage emotions and relationships, often finding herself in humorous and relatable situations that resonate with readers."
    },
    {
        id: 3,
        content: "Throughout the story, Operation True Love not only explores Seojin's pursuit of Sunjin but also delves into the dynamics of friendship and personal growth. Seojin's friends play pivotal roles in her journey, offering both encouragement and comic relief as they help her navigate the ups and downs of adolescent romance."
    },
    {
        id: 4,
        content: "As the plot progresses, Seojin learns valuable lessons about love, self-confidence, and the importance of being true to oneself. The webtoon strikes a balance between lighthearted humor and heartfelt moments, capturing the essence of teenage life with its mix of insecurities, aspirations, and budding emotions."
    },
    {
        id: 5,
        content: "Operation True Love ultimately celebrates the joy and challenges of first crushes and friendships, making it a delightful read for fans of romantic comedies and slice-of-life stories alike. With its endearing characters and engaging storyline, the webtoon resonates with readers who enjoy stories about youth, love, and the journey of self-discovery during the tumultuous yet memorable high school years."
    }
];




export const navLinks = [
    { text: 'Home', href: `${DOMAIN}` },
    { text: 'DMCA', href: `${DOMAIN}/dmca` },
    { text: 'Terms & Conditions', href: `${DOMAIN}/terms-and-conditions` },
];

export const FooterLinks = [
    { text: 'About', href: `${DOMAIN}/about` },
    { text: 'Contact', href: `${DOMAIN}/contact` },
    { text: 'Disclaimer', href: `${DOMAIN}/disclaimer` },
    { text: 'Privacy Policy', href: `${DOMAIN}/privacy-policy` },
    { text: 'Terms & Conditions', href: `${DOMAIN}/terms-and-conditions` }
];



export const RelatedMangaLinks = [
    {
        imageUrl: `${DOMAIN}/related/whomademeaprincess.webp`,
        title: 'Who Made Me A Princess',
        link: 'https://www.whomademeaprincess.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/warriorhighschool.webp`,
        title: 'Warrior High School',
        link: 'https://www.warriorhighschool.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thislifestartsasachildactor.webp`,
        title: 'This Life Starts As a Child Actor',
        link: 'https://www.thislifestartsasachildactor.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thevillaninsfavouriteisme.webp`,
        title: 'The Villanins Favourite is Me',
        link: 'https://www.thevillaninsfavouriteisme.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thegreatestestatedeveloper.webp`,
        title: 'The Greatest Estate Developer',
        link: 'https://www.thegreatestestatedeveloper.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thedivorceesdessertcafe.webp`,
        title: 'The Divorcees Dessert Cafe',
        link: 'https://www.thedivorceesdessertcafe.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thebeginningaftertheend.webp`,
        title: 'The Beginning After The End',
        link: 'https://www.thebeginningaftertheend.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/talentedbabysquirrel.webp`,
        title: 'Talented Baby Squirrel',
        link: 'https://www.talentedbabysquirrel.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/solomaxlevelnewbie.webp`,
        title: 'Solo Max Level Newbie',
        link: 'https://www.solomaxlevelnewbie.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/secondliferanker.webp`,
        title: 'Second Life Ranker',
        link: 'https://www.secondliferanker.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/operationtruelove.webp`,
        title: 'Operation True Love',
        link: 'https://www.operationtruelove.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/omniscientreadersviewpoint.webp`,
        title: 'Omniscient Readers Viewpoint',
        link: 'https://www.omniscientreadersviewpoint.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/myschoollifepretendingtobeaworthlessperson.webp`,
        title: 'My School Life Pretending to be a Worthless Person',
        link: 'https://www.myschoollifepretendingtobeaworthlessperson.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/kallithechampion.webp`,
        title: 'Kalli The Champion',
        link: 'https://www.kallithechampion.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/ishallmasterthisfamily.webp`,
        title: 'I Shall Master This Family',
        link: 'https://www.ishallmasterthisfamily.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/iobtainedamythicitem.webp`,
        title: 'I Obtained A Mythic Item',
        link: 'https://www.iobtainedamythicitem.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/goldenlightgratiathechildlovedbygod.webp`,
        title: 'Golden Light Gratia The Child Loved By God',
        link: 'https://www.goldenlightgratiathechildlovedbygod.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/disastrouscatastrophicnecromancer.webp`,
        title: 'Disastrous Catastrophic Necromancer',
        link: 'https://www.disastrouscatastrophicnecromancer.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/adoptedbyamurderousdukefamily.webp`,
        title: 'Adopted by a Murderous Duke Family',
        link: 'https://www.adoptedbyamurderousdukefamily.in.net',
    },
];



export const chaptersData = [
    {
        "chapterNumber": "0",
        "numImages": 23
    },
    {
        "chapterNumber": "1",
        "numImages": 32
    },
    {
        "chapterNumber": "2",
        "numImages": 53
    },
    {
        "chapterNumber": "3",
        "numImages": 35
    },
    {
        "chapterNumber": "4",
        "numImages": 56
    },
    {
        "chapterNumber": "5",
        "numImages": 45
    },
    {
        "chapterNumber": "6",
        "numImages": 3
    },
    {
        "chapterNumber": "7",
        "numImages": 15
    },
    {
        "chapterNumber": "8",
        "numImages": 2
    },
    {
        "chapterNumber": "9",
        "numImages": 2
    },
    {
        "chapterNumber": "10",
        "numImages": 2
    },
    {
        "chapterNumber": "11",
        "numImages": 3
    },
    {
        "chapterNumber": "12",
        "numImages": 3
    },
    {
        "chapterNumber": "13",
        "numImages": 18
    },
    {
        "chapterNumber": "14",
        "numImages": 12
    },
    {
        "chapterNumber": "15",
        "numImages": 12
    },
    {
        "chapterNumber": "16",
        "numImages": 6
    },
    {
        "chapterNumber": "17",
        "numImages": 16
    },
    {
        "chapterNumber": "18",
        "numImages": 10
    },
    {
        "chapterNumber": "19",
        "numImages": 16
    },
    {
        "chapterNumber": "20",
        "numImages": 22
    },
    {
        "chapterNumber": "21",
        "numImages": 31
    },
    {
        "chapterNumber": "22",
        "numImages": 5
    },
    {
        "chapterNumber": "23",
        "numImages": 16
    },
    {
        "chapterNumber": "24",
        "numImages": 4
    },
    {
        "chapterNumber": "25",
        "numImages": 7
    },
    {
        "chapterNumber": "26",
        "numImages": 7
    },
    {
        "chapterNumber": "27",
        "numImages": 21
    },
    {
        "chapterNumber": "28",
        "numImages": 6
    },
    {
        "chapterNumber": "29",
        "numImages": 9
    },
    {
        "chapterNumber": "30",
        "numImages": 12
    },
    {
        "chapterNumber": "31",
        "numImages": 9
    },
    {
        "chapterNumber": "32",
        "numImages": 14
    },
    {
        "chapterNumber": "33",
        "numImages": 13
    },
    {
        "chapterNumber": "34",
        "numImages": 2
    },
    {
        "chapterNumber": "35",
        "numImages": 2
    },
    {
        "chapterNumber": "36",
        "numImages": 2
    },
    {
        "chapterNumber": "37",
        "numImages": 4
    },
    {
        "chapterNumber": "38",
        "numImages": 26
    },
    {
        "chapterNumber": "39",
        "numImages": 29
    },
    {
        "chapterNumber": "40",
        "numImages": 24
    },
    {
        "chapterNumber": "41",
        "numImages": 31
    },
    {
        "chapterNumber": "42",
        "numImages": 17
    },
    {
        "chapterNumber": "43",
        "numImages": 29
    },
    {
        "chapterNumber": "44",
        "numImages": 30
    },
    {
        "chapterNumber": "45",
        "numImages": 25
    },
    {
        "chapterNumber": "46",
        "numImages": 28
    },
    {
        "chapterNumber": "47",
        "numImages": 27
    },
    {
        "chapterNumber": "48",
        "numImages": 17
    },
    {
        "chapterNumber": "49",
        "numImages": 28
    },
    {
        "chapterNumber": "50",
        "numImages": 19
    },
    {
        "chapterNumber": "51",
        "numImages": 21
    },
    {
        "chapterNumber": "52",
        "numImages": 28
    },
    {
        "chapterNumber": "53",
        "numImages": 18
    },
    {
        "chapterNumber": "54",
        "numImages": 23
    },
    {
        "chapterNumber": "55",
        "numImages": 24
    },
    {
        "chapterNumber": "56",
        "numImages": 26
    },
    {
        "chapterNumber": "57",
        "numImages": 22
    },
    {
        "chapterNumber": "58",
        "numImages": 14
    },
    {
        "chapterNumber": "59",
        "numImages": 16
    },
    {
        "chapterNumber": "60",
        "numImages": 15
    },
    {
        "chapterNumber": "61",
        "numImages": 14
    },
    {
        "chapterNumber": "62",
        "numImages": 15
    },
    {
        "chapterNumber": "63",
        "numImages": 14
    },
    {
        "chapterNumber": "64",
        "numImages": 16
    },
    {
        "chapterNumber": "65",
        "numImages": 20
    },
    {
        "chapterNumber": "66",
        "numImages": 18
    },
    {
        "chapterNumber": "67",
        "numImages": 19
    },
    {
        "chapterNumber": "68",
        "numImages": 15
    },
    {
        "chapterNumber": "69",
        "numImages": 14
    },
    {
        "chapterNumber": "70",
        "numImages": 18
    },
    {
        "chapterNumber": "71",
        "numImages": 20
    },
    {
        "chapterNumber": "72",
        "numImages": 17
    },
    {
        "chapterNumber": "73",
        "numImages": 19
    },
    {
        "chapterNumber": "74",
        "numImages": 18
    },
    {
        "chapterNumber": "75",
        "numImages": 16
    },
    {
        "chapterNumber": "76",
        "numImages": 17
    },
    {
        "chapterNumber": "77",
        "numImages": 23
    },
    {
        "chapterNumber": "78",
        "numImages": 23
    },
    {
        "chapterNumber": "79",
        "numImages": 21
    },
    {
        "chapterNumber": "80",
        "numImages": 14
    },
    {
        "chapterNumber": "81",
        "numImages": 18
    },
    {
        "chapterNumber": "82",
        "numImages": 14
    },
    {
        "chapterNumber": "83",
        "numImages": 35
    },
    {
        "chapterNumber": "84",
        "numImages": 15
    },
    {
        "chapterNumber": "85",
        "numImages": 20
    },
    {
        "chapterNumber": "86",
        "numImages": 12
    },
    {
        "chapterNumber": "87",
        "numImages": 14
    },
    {
        "chapterNumber": "88",
        "numImages": 12
    },
    {
        "chapterNumber": "89",
        "numImages": 17
    },
    {
        "chapterNumber": "89a",
        "numImages": 2
    },
    {
        "chapterNumber": "89b",
        "numImages": 11
    },
    {
        "chapterNumber": "90",
        "numImages": 11
    },
    {
        "chapterNumber": "91",
        "numImages": 11
    },
    {
        "chapterNumber": "92",
        "numImages": 13
    },
    {
        "chapterNumber": "93",
        "numImages": 12
    },
    {
        "chapterNumber": "94",
        "numImages": 12
    },
    {
        "chapterNumber": "95",
        "numImages": 11
    },
    {
        "chapterNumber": "96",
        "numImages": 14
    },
    {
        "chapterNumber": "97",
        "numImages": 6
    },
    {
        "chapterNumber": "98",
        "numImages": 11
    },
    {
        "chapterNumber": "99",
        "numImages": 4
    },
    {
        "chapterNumber": "100",
        "numImages": 4
    },
    {
        "chapterNumber": "101",
        "numImages": 6
    }
]